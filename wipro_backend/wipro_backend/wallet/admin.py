...
#         "amount",
#         # "purpose",
#         "user_payment_method",          # 👈 NEW (user-chosen)
#         "payment_method",
#         "status",
#         "earned",
#         "paid",
...
class PaymentRequestAdmin(admin.ModelAdmin):
    list_display = (
        ...
        "request_type",
        "amount",
        "payment_method",              # backend-provided (deposit)
        "user_payment_method",          # 👈 NEW (user-chosen)
        "status",
        "earned",
        "paid",
    ...
    list_filter = (
        "request_type",
        "status",
        "user_payment_method",          # 👈 optional but useful
    )
    ...
    fieldsets = (
        ...
        ("Payment Method", {
            "fields": (
                "payment_method",        # system method (deposit)
                "user_payment_method",   # 👈 user choice (withdraw)
    ...
    # 🧮 Safety: auto-clear wrong side
    def save_model(self, request, obj, form, change):
        if obj.request_type == "deposit":
            obj.paid = 0
